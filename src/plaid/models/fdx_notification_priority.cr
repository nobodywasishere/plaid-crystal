...
module Plaid
  class FDXNotificationPriority
    ...
    ERROR_MESSAGE = %{invalid value for "FDXNotificationPriority", must be one of ["HIGH", "MEDIUM", "LOW"].}

    VALID_VALUES = String.static_array("HIGH", "MEDIUM", "LOW")

    delegate to_json_object_key, to: @data

    def self.from_json(value : JSON::PullParser) : FDXNotificationPriority
      new(value)
    end

    def self.to_json(value : FDXNotificationPriority, json : JSON::Builder) : Nil
      value.to_json(json)
    end
    ...
    def validate : Nil
      OpenApi::EnumValidator.validate("FDXNotificationPriority", data, VALID_VALUES, false)
    end
