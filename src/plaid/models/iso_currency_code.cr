...
module Plaid
  class ISOCurrencyCode
    ...
    ERROR_MESSAGE = %{invalid value for "ISOCurrencyCode", must be one of ["USD"].}

    VALID_VALUES = String.static_array("USD")

    delegate to_json_object_key, to: @data

    def self.from_json(value : JSON::PullParser) : ISOCurrencyCode
      new(value)
    end

    def self.to_json(value : ISOCurrencyCode, json : JSON::Builder) : Nil
      value.to_json(json)
    end
    ...
    def validate : Nil
      OpenApi::EnumValidator.validate("ISOCurrencyCode", data, VALID_VALUES, false)
    end
